...
contract tRBTCVault is ERC4626, Ownable {
    ...
    IERC20 public investmentToken; // tRBTC Token
    IUniswapV2Router public dexRouter; // Uniswap V2 Router
    IERC20 public governance;
    ...
    constructor(IERC20 _WtRBTCToken, IUniswapV2Router _dexRouter, IERC20 governance_Token) ERC4626(_WtRBTCToken) ERC20("tRBTC Token", "tRBTC") Ownable(msg.sender) {
        investmentToken = _WtRBTCToken;
        dexRouter = _dexRouter;
        governance = governance_Token;

        // DOMAIN_SEPARATOR = keccak256(abi.encode(
        //     keccak256("EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)"),
        //     keccak256(bytes("tRBTCVault")),
        //     keccak256(bytes("1")),
        //     block.chainid,
        //     address(this)
        // ));
    ...
    function withdraw(uint256 assets, address receiver, address _owner) public override returns (uint256 shares) {
        ...
        for (uint i = 0; i < investments[_owner].length; i++) {
            ...
            totalFee += fee;
            totalRIF += (investments[_owner][i].depositTime - block.timestamp)*(investments[_owner][i].amount);
        ...
        governance.transfer(receiver, totalRIF * 1e15);
        // Rest of the withdraw logic
        return super.withdraw(assets, receiver, _owner);
    }

    function depositAndConvert(address tokenAddress, uint256 assets) public {
        require(tokenAddress != address(investmentToken), "tRBTC deposit not allowed");
        ...
        // Define the swap path (token -> tRBTC)
        ...
        uint256 amounttRBTC = balanceAfterSwap - balanceBeforeSwap;

        // Mint vault shares to the depositor based on the amount of tRBTC received
        _mint(msg.sender, amounttRBTC);
    }
...
/*
The feeDecreaseInterval variable in the tRBTCVault contract represents the time interval after which the fee percentage will decrease. Let's break down its purpose and how it works:

Purpose: The purpose of the feeDecreaseInterval is to gradually decrease the fee percentage charged by the tRBTCVault contract over time. By decreasing the fee percentage, the contract aims to incentivize long-term investments and provide more favorable terms for investors.
...
The decreased fee percentage will then be applied to future transactions or operations within the tRBTCVault contract.
...
Overall, the feeDecreaseInterval variable allows the tRBTCVault contract to gradually reduce the fee percentage over time, providing potential benefits to investors and encouraging long-term participation in the investment platform.
